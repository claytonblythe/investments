...
saveGIF((
  for (i in seq(from=1, to=481, by=1)){
    ...
    max <- round(max(performance[,i],na.rm=TRUE),2)
    min <- round(min(performance[,i],na.rm=TRUE),2)
    median <- median(performance[,i],na.rm=TRUE)
    ...
    text(x=16.5, y=.45, labels=paste("Median: ", format(round(mode, 2),nsmall=2),  sep=""), col='gray3', cex =1.5)
    ...
    text(x=16.5, y=.37, labels=paste("Max:  ", format(round(max, 2),nsmall=2),  sep=""), col='gray3', cex =1.5)
    text(x=16.5, y=.35, labels=paste("Min:  ", format(round(min, 2),nsmall=2),  sep=""), col='gray3', cex =1.5)
    abline(v=avg, col="chartreuse3", lwd=3, lty=2)
    abline(v=median, col="black", lwd=3, lty=2)
  }
...
# ggsave("cashmultipliers_Spectral_10years.pdf", plot=last_plot())
# 
# 
...
relevant_years <- dplyr::filter(performance_melted, time_horizon %% 10 == 0 & time_horizon > 0 & time_horizon <=40)
